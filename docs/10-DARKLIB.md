...
| **5** | الفك: **5a** AVIF برمجي في النواة عبر **rav1d** (لون/شفافية/اتجاه/شبكات ImageGrid) + تبليط الترميز (>16MP) + حمل HDR عبر تحويل AVIF→AVIF — **مُنجَز**؛ **5b** فكّ HEIC **هجين**: جسر فكّ المنصّة (bakeUpright/ImageIO على iOS، BitmapFactory ضمن مسارَي العتاد وfic على أندرويد) → DarkLib يرمّز ويحمل الميتاداتا — مكتمل وظيفيًا؛ نقله خلف واجهة Rust + مسرّع MediaCodec `video/av01` = تحسين اختياري لاحق | **مُنجَز (هجين)** |
| **6** | مهاجرة الضغط/التحويل إلى DarkLib — **مُنجَزة الأساس**: AVIF (عتاد + زرع ميتاداتا → DarkLib → جسر HEIC → flutter_avif) وWebP (DarkLib أولًا + جسر HEIC — سُدّت فجوة iOS) داخل `ImageEncoder` فيرثه كل المستدعين والتقدير؛ **observability**: تسجيل تدهور المسارات في بناء التطوير؛ **golden CI**: `.github/workflows/ci.yml` (cargo fmt/clippy/test + flutter analyze/test)؛ **بند مستقبلي واحد:** بثّ فكّ المصادر العملاقة لرفع سقف 8192 (السقف باقٍ عمدًا — أمان الذاكرة أولًا) | **مُنجَزة (الأساس)** |
