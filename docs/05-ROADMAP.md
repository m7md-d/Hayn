...
- **فهرس ميديا على الجهاز (drift/SQLite)** + قناة native للحجم بالبايت (Android `MediaStore._size` / iOS `PHAssetResource.fileSize`) — يجعل الفرز/الفلترة تشمل المكتبة كاملة لا الصفحات المحمَّلة فقط. (سُحب مبكّرًا من خطة الـ DB في المرحلة الجراحية لأنه أساس تصفّح.)
- صلابة أداء التصفّح: `ConcurrencyLimiter` يحدّ طلبات المصغّرات/الأحجام + إلغاء للمربّعات المنفصلة + memoization لقائمة العرض + حصر انميشن الدخول في الشاشة الأولى.
...
> **إضافة (سحب مبكّر من خطة الـ DB):** فهرس ميديا على الجهاز بـ **drift/SQLite** يخزّن البيانات الخفيفة لكل أصل (id/النوع/التواريخ/الأبعاد/المدة/**الحجم**/الصيغة) — بضعة ميجابايت لـ10 آلاف. الفرز/الفلترة على المكتبة كاملة تصير `ORDER BY/WHERE` مفهرسة (لحظية وصحيحة)، بينما التصفّح الافتراضي (الأحدث، بلا فلتر) يبقى على مسار `photo_manager` السريع. الحجم بالبايت يُجلب عبر قناة native بلا فتح الملف (مع fallback آمن إلى `asset.file`). الفهرس يُحدّث عند الإقلاع وعند تغيّر المكتبة (change-callback). تفاصيل في `02-ARCHITECTURE.md`.
